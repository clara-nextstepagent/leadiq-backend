    ...
    if (text.includes("QUALIFICATION_COMPLETE:")) {
      try {
        ...
        if (qualif.scores) {
          ...
          qualif.score = Math.round(((s.budget||0) + (s.urgence||0) + (s.projet||0) + (s.financement||0)) / 4);
        ...
        return res.json({ type: "qualification", data: {
          ...
          budget: "non précisé", financement: "non précisé",
          delai: "urgent", analyse: "Prospect qualifié — détails à confirmer.",
          scores: { budget: 60, urgence: 100, projet: 60, financement: 0 }
        }});
      }
    }
